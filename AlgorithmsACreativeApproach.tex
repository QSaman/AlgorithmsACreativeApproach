...
uint64_t fibonacci(int n) //O(logn)
{
  ...
  return res;
	...
	\begin{algorithm*}[h!]
		\begin{algorithmic}[1]
			\Function{Eear-Decomposition}{$G$}
				\State \Call{Init}{$G$} \Comment{$O(V)$}
				\ForAll{$u \in G.V$}
					\If{$u.color == WHITE$} \Comment{$O(V)$}
						\State Let $CC$ be a new connected component \Comment{$O(CCs)$}
						\State $CC.V.clear()$
						\State $CC.E = \emptyset$
						\State \Call{DFS}{$G, u, CC$} \Comment{$O(E)$}
						\State \Call{Find-Ears}{$CC, u$} \Comment{$O(V + E)$}
		...
		\begin{algorithmic}[1]		
			\Function{Init}{$G$}
				\ForAll{$u \in G.V$}
					\State $u.color = WHITE$
					\State $u.\pi = NIL$
					\State $u.visited = False$ \Comment{belong to previous ears}
				\EndFor
				\State $G.CCs = \emptyset$ \Comment{connected components}		
			\EndFunction
		\end{algorithmic}
	\end{algorithm*}	
	\FloatBarrier
	...
	\begin{algorithm*}[h!]
		\begin{algorithmic}[1]
			\Function{Find-Ears}{$CC, u$}
				\State $CC.bridges = \emptyset$
				\ForAll{$(u, v) \in CC.E$}
					\State $CC.bridges = CC.bridges \cup \{(u, v)\}$
				\EndFor
				\ForAll{$u \in CC.V$} \Comment{the list is in DFS order}
					\ForAll{$v \in CC.adj[u]$}					
						\State tree-edge = $u.\pi == v \lor v.\pi == u$
						\State back-edge = tree-edge == $False$
						\If{back-edge $\land (u, v) \in CC.bridges$}
	...
	\begin{algorithm*}[h!]
		\begin{algorithmic}
			\Function{Find-Cut-Verticies}{$G$}
				\State \Call{Eear-Decomposition}{$G$}
				\ForAll{$CC \in G.CCs$}
					\State CC.cut-verticies = $\emptyset$
					\ForAll{$ear \in \{CC.ears - \{CC.ears.front()\}\}$} \Comment{$O(V)$}
					...
					\ForAll{$(u, v) \in CC.bridges \land (u, v) \notin\ $ visited-bridges} \Comment{$O(E)$}
						\If{$CC.adj[u].size() > 1$} \Comment{$v \in CC.adj[u]$}
							\State CC.cut-verticies = CC.cut-verticies $\cup\ u$
						\EndIf
						\If{$CC.adj[v].size() > 1$} \Comment{$u \in CC.adj[v]$}
