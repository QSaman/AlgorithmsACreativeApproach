...
int main()
{
  ...
  grayCodes(0, n);
	...
	\begin{equation*}
		...
		\end{split}
	\end{equation*}
	How this implementation always generate closed gray codes when $n$ is even? Suppose $n = 2m$. We always start with all bits set to zero ($0s_1$). The next code only differs in 1 digit from $0s_1$. The number after arrow is the number of different digits from $0s_1$:
	\begin{equation*}
		\begin{split}
		0s_1 &\rightarrow 0 \\
		0s_2 &\rightarrow 1 \\
		...
		0s_m &\rightarrow m - 1 \\
		1s_m &\rightarrow m \\
		1s_{m - 1}^{\prime} &\rightarrow m - 1 \\
		&\vdots \\
		1s_1^{\prime} &\rightarrow 1
		\end{split}
	\end{equation*}
